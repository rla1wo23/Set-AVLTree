...
public:
    ...
    /*show tree*/
    void print_helper(Node *root_, std::string indent, bool last)
    {
        ...
        if (root_ != nullptr)
        {
            ...
            int height = std::max(root_->left->height, root_->right->height) + 1;
            std::cout << root_->key << " (" << height << ")" << std::endl;
...
public:
    ...
    void Delete_helper()
    {
        ...
        this->root_ = EraseNode(this->root_, item);
        return;
    }
...
int main()
{
    ...
    tree.Insert(5);
    tree.Insert(8);
    tree.print_helper(tree.root_, "", false);
    return 0;
}